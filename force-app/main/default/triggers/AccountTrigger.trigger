...
/*if (trigger.isBefore&&Trigger.isInsert){
        ...
        /*if(trigger.isBefore&& trigger.isUpdate){
            List<Account> accTriggerOld=trigger.old;//old values record
            for(account oldAcc: accTriggerOld){
                system.debug('old acc name= '+ oldAcc.Name+ oldAcc.id);

            }
            List<Account> accTriggerNew=trigger.New;//new values record
            for(account newAcc: accTriggerOld){
                system.debug('new acc name= '+ newAcc.Name+ newAcc.id);

            }

        }if(trigger.isAfter){
            list<account>  accTriggerNew=trigger.new;//new updated value of records
            set<id> setIds=new set<id>();//add all ids of accounts which are inserted/updated
            for (account newAcc : accTriggerNew) {
                id accId=newAcc.Id;//adding id to newly created set
                setIds.add(accId);
            }
            system.debug(setids);
